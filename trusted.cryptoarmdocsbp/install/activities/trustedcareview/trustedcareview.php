...
if (class_exists("CBPTrustedCAReview")) {
    return;
}

class CBPTrustedCAReview
		...
		if (!array_key_exists("ShowComment", $arTask["PARAMETERS"]) || ($arTask["PARAMETERS"]["ShowComment"] != "N"))
		{
			...
			$form .=
				...
				if ($doc->getStatus() === DOC_STATUS_BLOCKED) {
					...
					$form .= '<tr><td colspan="2"><input type="button" class="ui-btn ui-btn-primary" style="border: none; height: 34px;line-height: 0;" value="' . GetMessage('BPAA_ACT_VERIFY') . '" onclick="trustedCA.verify([' . $lastDocId . '])" /></td></tr>';
				}
